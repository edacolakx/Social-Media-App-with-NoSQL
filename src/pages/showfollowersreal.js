...
export default function ShowfollowersReal() {
    ...
    useEffect(()=>{
       async function getFollowers() {
            try
            {
                const response= await axios.get('http://10.0.2.2:3000/register/'+genelResponse.kullaniciadi+"/follower")
                const data=response.data
                setData(data)
                console.log("takipçi",data)
            }
            catch(e){
                console.log(e)
            }
